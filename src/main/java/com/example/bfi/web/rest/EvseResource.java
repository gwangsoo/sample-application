...
/**
 * REST controller for managing {@link com.example.bfi.domain.Evse}.
 */
@RestController
...
public class EvseResource {
    ...
    public ResponseEntity<EvseDTO> createEvse(@Valid @RequestBody EvseDTO evseDTO) throws URISyntaxException {
        ...
        evseDTO = evseService.save(evseDTO);
        return ResponseEntity.created(new URI("/api/evses/" + evseDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, evseDTO.getId()))
            .body(evseDTO);
    }
    ...
    public ResponseEntity<EvseDTO> updateEvse(
        ...
        evseDTO = evseService.update(evseDTO);
        return ResponseEntity.ok()
            .headers(HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, evseDTO.getId()))
            .body(evseDTO);
    }
    ...
    public ResponseEntity<EvseDTO> partialUpdateEvse(
        ...
        return ResponseUtil.wrapOrNotFound(result, HeaderUtil.createEntityUpdateAlert(applicationName, true, ENTITY_NAME, evseDTO.getId()));
    }
     ...
     */
    @GetMapping("")
    public List<EvseDTO> getAllEvses() {
        log.debug("REST request to get all Evses");
        return evseService.findAll();
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<EvseDTO> getEvse(@PathVariable("id") String id) {
     ...
     */
    @DeleteMapping("/{id}")
    public ResponseEntity<Void> deleteEvse(@PathVariable("id") String id) {
