...
import com.phoenixdarts.toss.backend.service.RegionService;
...
/**
 * REST controller for managing {@link com.phoenixdarts.toss.backend.domain.Region}.
 */
@RestController
...
public class RegionResource {
    ...
    public ResponseEntity<RegionDTO> createRegion(@Valid @RequestBody RegionDTO regionDTO) throws URISyntaxException {
        ...
        return ResponseEntity.created(new URI("/api/regions/" + regionDTO.getId()))
            .headers(HeaderUtil.createEntityCreationAlert(applicationName, true, ENTITY_NAME, regionDTO.getId()))
            .body(regionDTO);
    }
     ...
     */
    @GetMapping("/{id}")
    public ResponseEntity<RegionDTO> getRegion(@PathVariable("id") String id) {
